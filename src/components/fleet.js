...
export function createFleet(scene) {
  ...
  const buildShip = () => {
    ...
    // Phong shading — per-fragment lighting with specular (metallic hull)
    const fuselage = new THREE.Mesh(
      fuseGeo,
      new THREE.MeshPhongMaterial({ color: 0x7799bb, shininess: 90 }),
    );
    ...
    const nose = new THREE.Mesh(
      noseGeo,
      new THREE.MeshPhongMaterial({ color: 0xaaccdd, shininess: 90 }),
    );
    nose.position.z = -5.5;
    nose.scale.y = 0.6;
    ship.add(nose);
    ...
    const wings = new THREE.Mesh(
      wingGeo,
      new THREE.MeshPhongMaterial({ color: 0x6688aa, shininess: 60 }),
    );
    ...
    [-6, 6].forEach((xPos) => {
      const pod = new THREE.Mesh(
        podGeo,
        new THREE.MeshPhongMaterial({ color: 0x558899, shininess: 60 }),
      );
      pod.rotation.x = Math.PI / 2;
      pod.position.set(xPos, 0, 2);
      ship.add(pod);
    });
