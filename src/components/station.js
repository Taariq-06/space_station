...
export function createStation(scene) {
  ...
  // --- Shading toggle ---
  // Collect all lit meshes on the station for runtime material swapping.
  // We skip MeshBasicMaterial meshes (beacon tips) — they are intentionally unlit.
  const litMeshes = [];
  spaceStation.traverse((obj) => {
    if (
      (obj.isMesh && obj.material.isMeshPhongMaterial) ||
      (obj.isMesh && obj.material.isMeshLambertMaterial)
    ) {
      litMeshes.push({
        mesh: obj,
   ...
   * Called by SceneManager when the player presses G.
   * @param {'phong'|'flat'|'gouraud'} mode
   */
